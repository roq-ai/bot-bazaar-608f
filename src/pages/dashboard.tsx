...
import { AiToolListPage as ShowCaseList } from 'pages/ai-tools';

function DashboardPage() {
  const { session } = useSession();

  const pluginStat = useSWR<PaginatedInterface<PluginInterface>>(
    () => `/plugins`,
    () => getPlugins({ limit: 1, offset: 0 }),
  );
  ...
  const aiToolStat = useSWR<PaginatedInterface<AiToolInterface>>(
    () => `/ai-tools`,
    () => getAiTools({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Plugins',
      result: pluginStat?.data?.totalCount,
      isLoading: pluginStat.isLoading,
    ...
    {
      title: 'Ai Tools',
      result: aiToolStat?.data?.totalCount,
      isLoading: aiToolStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`ai-tools`} passHref legacyBehavior>
                  <Button
